...
class AdsPackageResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'Advertisement Package';
                                        ...
                                        ->schema([
                                            Forms\Components\Grid::make(3)
                                                ->schema(function ($record) use ($currencies) {
                                                    $options = AdsOption::with('Currencies')->get();
                                                    $schema = [];

                                                    // Get existing options for this record
                                                    $existingOptions = [];
                                                    if ($record) {
                                                        $existingOptions = $record->AdsOptions->pluck('id')->toArray();
                                                    }
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('title')
                    ...
                    ->description(
                        fn($record): string =>
                        $record->AdsOptions()->count() . ' options'
                    ),
                ...
                Tables\Columns\TextColumn::make('options_info')
                    ->label('Options')
                    ->getStateUsing(function ($record): string {
                        if ($record->AdsOptions->isEmpty()) {
                            return 'No options';
                        }

                        $optionNames = $record->AdsOptions->take(3)->pluck('title')->implode(', ');
                        return $optionNames . ($record->AdsOptions->count() > 3 ? '...' : '');
                    })
                    ->badge()
                    ->color('gray')
                    ->toggleable(),
