                    ...
                    @foreach ($package->AdsOptions as $option)
                        ...
                        <tr class="text-[8px]">
                            <td  class="w-2/6 text-[8px]">
                                <input disabled type="checkbox" @checked(in_array($package->id . '_' . $option->id, $contract->ads_check ?? [])) class="mr-1 h-3 text-[8px]">{{ $option->title }}
                            </td>
                            ...
                            @if (!$loop->last)
                                <td  class="w-2/6 text-[8px]">
                                    <input disabled type="checkbox" @checked(in_array($package->id . '_' . $package->AdsOptions[$loop->index + 1]->id, $contract->ads_check ?? []))
                                        class="mr-1 h-3 text-[8px]">{{ $package->AdsOptions[$loop->index + 1]->title }}
                                </td>
                                <td  class="w-1/6 text-[8px]">
