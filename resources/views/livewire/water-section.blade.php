<div class="flex justify-between pt-[1px]">
    <div class="flex justify-end border border-black w-3/4 mr-6">
        <div class="w-1/2 items-center text-xs gap-2 pb-[2px]">
            <input disabled type="checkbox" class="mr-2" @checked($contract->if_water)>
            <label class="font-bold">Water point needed (if available)</label>
        </div>
        <div class="w-1/2 items-center text-xs gap-2 pb-[2px]">
            <input disabled type="checkbox" class="mr-2" @checked($contract->if_electricity)>
    ...
    <div class="w-1/4">
        <div class="text-right font-bold border border-black pb-[2px]">
            <p>Total: {{$contract->water_electricity_amount}} {{$currency->CODE}}</p>
        </div>
    </div>
</div>
