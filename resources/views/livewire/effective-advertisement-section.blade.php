            ...
            @foreach ($event->EffAdsPackages as $package)
                ...
                <table class="w-full">
                    @foreach ($package->EffAdsOptions as $option)
                        ...
                        <tr class="text-xs">
                            <td class="w-2/6">
                                <input type="checkbox" @checked(in_array($package->id . '_' . $option->id, $contract->eff_ads_check ?? [])) class="mr-1">{{ $option->title }}
                            </td>
                            <td class="w-1/6">
                            ...
                            @if (!$loop->last)
                                <td class="w-2/6">
                                    <input type="checkbox" @checked(in_array($package->id . '_' . $package->EffAdsOptions[$loop->index + 1]->id, $contract->eff_ads_check ?? []))
