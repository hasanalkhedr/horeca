...
@section('content')
    <div class="container mx-auto px-4 py-8" x-data="packageShow()">
        <div class="flex justify-between items-center mb-6">
            <div>
                <a href="{{ route('eff-ads-packages.index') }}" class="text-blue-500 hover:text-blue-700 flex items-center">
                    <i class="fas fa-arrow-left mr-2"></i> Back to Packages
                </a>
            </div>
            <div class="flex space-x-3">
                <button @click="openModal('edit', {{ $adsPackage }})"
                        class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded">
                    <i class="fas fa-edit mr-2"></i>Edit
                </button>
                <button @click="openModal('delete', {{ $adsPackage }})"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden mb-8">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-gray-800">{{ $adsPackage->title }}</h2>
                <p class="text-gray-600 mt-2">{{ $adsPackage->description }}</p>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'currencies'" class="bg-white rounded-lg shadow overflow-hidden mb-8">
            ...
            @if($adsPackage->currencies->count() > 0)
                <div class="divide-y divide-gray-200">
                    @foreach($adsPackage->currencies as $currency)
        ...
        <div x-show="activeTab === 'options'" class="bg-white rounded-lg shadow overflow-hidden mb-8">
            ...
            @if($adsPackage->effAdsOptions->count() > 0)
                <div class="divide-y divide-gray-200">
                    @foreach($adsPackage->effAdsOptions as $option)
                        <div class="px-6 py-4 flex justify-between items-center">
                            <div>
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea id="edit_description" x-model="formData.description" rows="3"
    ...
    <script>
        function packageShow() {
            return {
                ...
                selectedPackageId: {{ $adsPackage->id }},
                selectedPackage: @json($adsPackage),
                ...
                addCurrency() {
                    ...
                    fetch(`/eff-ads-packages/${this.selectedPackageId}/add-currency`, {
                        method: 'POST',
                        headers: {
                ...
                removeCurrency(currencyId) {
                    ...
                    fetch(`/eff-ads-packages/${this.selectedPackageId}/remove-currency/${currencyId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
                ...
                addOption() {
                    ...
                    fetch(`/eff-ads-packages/${this.selectedPackageId}/attach-option`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        ...
                        body: JSON.stringify({
                            eff_ads_option_id: this.newOption.id
                        })
                    })
                    .then(response => {
                        if (!response.ok) {
                ...
                removeOption(optionId) {
                    ...
                    fetch(`/eff-ads-packages/${this.selectedPackageId}/detach-option/${optionId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
                ...
                submitForm() {
                    ...
                    fetch(`/eff-ads-packages/${this.selectedPackageId}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                confirmDelete() {
                    ...
                    .then(() => {
                        window.location.href = '{{ route("eff-ads-packages.index") }}';
                    })
                    .catch(error => {
