...
@section('content')
    <div class="container mx-auto px-4 py-8" x-data="packageTable()">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Effective Advertisement Packages</h1>
        ...
        <div class="mb-6 bg-white p-4 rounded-lg shadow">
            <form action="{{ route('eff-ads-packages.index') }}" method="GET" class="flex items-center space-x-4">
                <div class="flex-1">
                ...
                @if(request('search'))
                    <a href="{{ route('eff-ads-packages.index') }}"
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route('eff-ads-packages.index', ['sort' => 'id', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
                                ID {!! request('sort') == 'id' ? (request('direction') == 'asc' ? '↑' : '↓') : '' !!}
                            </a>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route('eff-ads-packages.index', ['sort' => 'title', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($packages as $package)
                        <tr class="hover:bg-gray-50 cursor-pointer"
                            onclick="window.location.href='{{ route('eff-ads-packages.show', $package->id) }}'">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @foreach($package->effAdsOptions as $opt)
                                    <span class="inline-block bg-gray-100 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 mr-1 mb-1">
                                        {{ $opt->title }}
                                    </span>
                                @endforeach
            ...
            return {
                ...
                submitForm() {
                    ...
                    const url = this.action === 'add'
                        ? '{{ route("eff-ads-packages.store") }}'
                        : '{{ route("eff-ads-packages.update", "") }}/' + this.selectedPackageId;
                ...
                confirmDelete() {
                    fetch('{{ route("eff-ads-packages.destroy", "") }}/' + this.selectedPackageId, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
