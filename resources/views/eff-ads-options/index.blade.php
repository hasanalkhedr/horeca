...
@section('content')
    <div class="container mx-auto px-4 py-8" x-data="optionTable()">
        <div class="flex">
            <h1 class="w-1/2 text-2xl font-bold mb-4">Effective Advertisement Options</h1>
        ...
        <!-- Search Bar -->
        <div class="mb-4">
            <form action="{{ route('eff-ads-options.index') }}" method="GET" class="flex-1">
            ...
            <thead class="bg-gray-200">
                <tr>
                    <th class="py-3 px-4 text-left">
                        <a href="{{ route('eff-ads-options.index', ['sort' => 'id', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
                            ID {!! request('sort') == 'id' ? (request('direction') == 'asc' ? '&#9650;' : '&#9660;') : '' !!}
                        </a>
                    </th>
                    <th class="py-3 px-4 text-left">
                        <a href="{{ route('eff-ads-options.index', ['sort' => 'title', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
            ...
            <tbody>
                @foreach ($options as $option)
                    <tr class="border-b {{ $loop->even ? 'bg-gray-50' : '' }} hover:bg-gray-100"
                        onclick="window.location.href='{{ route('eff-ads-options.show', $option->id) }}'">
            ...
            return {
                ...
                submitForm() {
                    ...
                    const url = this.action === 'add' ?
                        `{{ route('eff-ads-options.store') }}` :
                        `{{ route('eff-ads-options.update', '') }}/${this.selectedOptionId}`;
