                ...
                <div class="w-full px-3">
                    ...
                    <x-select-input name="stand_id" id="stand_id" required>
                        ...
                        @foreach ($stands as $stand)
                            <option value="{{ $stand->id }}" @selected($stand->id==$contract->stand_id)>{{ $stand->no }}|{{ $stand->space }}</option>
                        @endforeach
                    </x-select-input>
                </div>
                <div class="w-full px-3">
                    ...
                    <div class="block">
                        ...
                        <input type="radio" name="price_id" value="0" id="special_price_radio" @checked($contract->price_id == null)
                        onclick="toggleAmountInput(this)" />Special pavilion specify
                        <input
                            class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                            name="special_price_amount" id="special_price_amount" type="number" step="0.01" @disabled($contract->price_id != null)
        ...
        @if (in_array('water-section', $report->components))
            ...
            <label class="inline-flex items-center">
                <input type="checkbox" name="if_electricity" class="text-indigo-600 focus:ring-indigo-500 border-gray-300"
                    value="1" @checked($contract->if_electricity) id="if_electricity" onclick="toogleElectricityAmount(this)" /> Extra electricity
                <input
                    class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                    name="electricity_text" id="electricity_text" type="text" value="{{$contract->electricity_text}}" placeholder="WATT Needed" disabled />
                <input
                    class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                    name="water_electricity_amount" placeholder="Water & electricity Amount" type="number"
                    step="0.01" value="{{$contract->water_electricity_amount}}"/>
                <script>
                ...
                <div class="w-full px-3">
                    ...
                    <x-select-input name="sponsor_package_id" id="sponsor_package_id">
                        ...
                        @foreach ($sponsor_packages as $package)
                            <option value="{{ $package->id }}" @selected($package->id==$contract->sponsor_package_id)>{{ $package->title }}|{{ $package->total_price }}
            ...
            <div class="w-full px-3">
                <x-select-input name="seller" id="seller">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @selected($contract->seller==$user->id)>{{ $user->name }}|{{ $user->getRoleNames() }}
                            </option>
                    @endforeach
                </x-select-input>
            </div>
