                        ...
                        @foreach ($sponsor_packages as $package)
                            <option value="{{ $package->id }}" data-price="{{ $package->currencies->where('id',$report->Currency->id)->first() ?
                                ...
                                {{ $package->title }}|{{ $package->currencies->where('id',$report->Currency->id)->first() ?
                                    $package->currencies->where('id',$report->Currency->id)->first()->pivot->total_price : 0}} {{ $report->Currency->CODE }}
                            </option>
                        @endforeach
                    </x-select-input>
                </div>
            ...
            <div class="flex">
                                ...
                                <tr class="text-xs">
                                    <td class="w-2/6">
                                        <input onchange="calculateTotal()" name="ads_check[]"
                                            ...
                                            @checked(in_array($package->id.'_'.$option->id, $contract->ads_check))
                ...
                <div class="w-1/4 total text-center p-2">
                    <div class="text-lg font-bold  bg-gray-200">
                        Advertisement Total: <span class="" id="ads_total">{{$contract->advertisment_amount}}</span> {{ $report->Currency->CODE ?? 'USD' }}
                        <input type="hidden" name="advertisment_amount" id="advertisment_amount" value="{{$contract->advertisment_amount}}">
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="flex">
            ...
            <div class="w-1/2 total pt-4 py-3 text-center">
                <div class="text-2xl font-bold  bg-gray-200">
                    Total: <span id="totalCost">{{ $contract->sub_total_2 ?? 0 }}</span> {{ $report->Currency->CODE ?? 'USD' }}
                </div>
            </div>
