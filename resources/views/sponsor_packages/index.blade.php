...
@section('content')
    <div class="container mx-auto px-4 py-8" x-data="packageTable()">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Sponsorship Packages</h1>
            <button @click="openModal('add')" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
        ...
        <div class="mb-6 bg-white p-4 rounded-lg shadow">
            <form action="{{ route('sponsor_packages.index') }}" method="GET" class="flex items-center space-x-4">
                <div class="flex-1">
                ...
                @if (request('search'))
                    <a href="{{ route('sponsor_packages.index') }}" class="text-gray-500 hover:text-gray-700">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a
                                href="{{ route('sponsor_packages.index', ['sort' => 'id', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
                                ID {!! request('sort') == 'id' ? (request('direction') == 'asc' ? '↑' : '↓') : '' !!}
                            </a>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a
                                href="{{ route('sponsor_packages.index', ['sort' => 'title', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($packages as $package)
                        <tr class="hover:bg-gray-50 cursor-pointer"
                            onclick="window.location.href='{{ route('sponsor_packages.show', $package->id) }}'">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @foreach ($package->currencies as $currency)
                                    <span
                                        class="inline-block bg-gray-100 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 mr-1 mb-1">
                                        {{ $currency->CODE }}: {{ $currency->pivot->total_price }}
                                    </span>
                                @endforeach
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @foreach ($package->sponsorOptions as $opt)
                                    <span
                                        class="inline-block bg-gray-100 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 mr-1 mb-1">
                                        {{ $opt->title }}
                                    </span>
        ...
        <div x-show="isOpen" @click.away="closeModal()" @keydown.escape.window="closeModal()" x-cloak
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-screen overflow-y-auto">
                ...
                <div x-show="action === 'delete'" class="p-6">
                    <p class="text-gray-700">Are you sure you want to delete this package? This action cannot be undone.
                    </p>
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button" @click="closeModal()"
                            class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            ...
            return {
                ...
                submitForm() {
                    ...
                    const url = this.action === 'add' ?
                        '{{ route('sponsor_packages.store') }}' :
                        '{{ route('sponsor_packages.update', '') }}/' + this.selectedPackageId;
                ...
                confirmDelete() {
                    fetch('{{ route('sponsor_packages.destroy', '') }}/' + this.selectedPackageId, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json'
